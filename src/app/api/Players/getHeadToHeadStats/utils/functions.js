export function getPlayerHeadToHeadStats(balls, playerId, opponentId) {
  ...
  const battingBalls = balls.filter((ball) => ball.strikerId === playerId && ball.bowlerId === opponentId)
  ...
  const battingStrikeRate = battingBallsFaced > 0 ? (battingRuns / battingBallsFaced) * 100 : 0

  const battingAverage = battingBallsFaced * 6 > 0 ? battingRuns / (battingBallsFaced * 6) : battingRuns

  const boundaryRuns = battingFours * 4 + battingSixes * 6

  const boundaryPercentage = battingRuns > 0 ? (boundaryRuns / battingRuns) * 100 : 0

  const bowlingBalls = balls.filter((ball) => ball.bowlerId === playerId && ball.strikerId === opponentId)
  ...
  const economy = decimalOvers > 0 ? bowlingRunsConceded / decimalOvers : 0

  const bowlingAverage = bowlingWickets > 0 ? bowlingRunsConceded / bowlingWickets : 0

  const bowlingStrikeRate = bowlingWickets > 0 ? bowlingLegalBalls / bowlingWickets : 0

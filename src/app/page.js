...
export default function Home() {
  ...
  function sratMatch_handler(e) {
    ...
    if (has_matchIn_LS && balls.length > 0) {
      setPopup((prev) => ({
        ...prev,
        visiable: true,
        matchDetails: matchDetails_setter(teams, overs),
      }))
    } else {
      dispatch(startInnings_fn())
      router.push("/selectTeamToStartMatch")
  ...
  return (
    <>
      ...
      {popup?.visiable && <StartScoringModal popup={popup} setPopup={setPopup} />}
      <div className="bg-background-light dark:bg-background-dark min-h-screen flex flex-col text-slate-900 dark:text-slate-100">
        ...
        <BottomNavigation sratMatch_handler={sratMatch_handler}></BottomNavigation>
      </div>
    </>
  )
}
      ...
      <DialogContent showCloseButton={false} className="sm:max-w-md p-0 overflow-hidden rounded-2xl">
        ...
        <div className="p-6">
          <DialogHeader className="mb-6">
            <DialogTitle className="text-xl font-bold">Start Scoring</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-semibold">Resume Previous Match</p>

                {popup.matchDetails && (
                  <p className="mt-1 text-sm text-muted-foreground">
                    <div>
                      {" "}
                      {popup?.matchDetails?.teamAName} <span className="text-red-500">VS</span>{" "}
                      {popup?.matchDetails?.teamBName}{" "}
                    </div>
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm opacity-80">Configure fresh scorecard</p>
              </div>
    ...
    <section className="p-4">
      <Card className="relative overflow-hidden bg-primary text-white p-6 rounded-xl shadow-lg shadow-primary/20">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl font-bold mb-2">Ready for a game?</h2>
    ...
    <header className="sticky top-0 z-10 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md px-4 py-4 flex items-center justify-between border-b border-primary/10">
      <div className="flex items-center gap-3">
        ...
        <div className="md:block hidden">
          ...
          <h1 className="text-lg font-bold text-primary dark:text-emerald-400">Welcome, Rahul</h1>
        </div>
      </div>

      <div className="flex gap-2">
        <button className="p-2 rounded-full hover:bg-primary/5 text-primary">
          <Link href="/settings" className="material-symbols-outlined">
            <Settings />
          </Link>
        </button>
      </div>

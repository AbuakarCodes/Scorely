...
//   const [fH2Hstate, setH2Hstate] = useState({ error: "", loading: false, data: {} })
...
//       (P) => P._id !== playerId && P?.name?.toLowerCase()?.includes(search?.toLowerCase()),
//     )
//   }, [search, playerId])
...
//   async function selectOpponent(opponent, setH2Hstate) {
//     await getHeadToHead_APi(playerId, opponent._id, setH2Hstate)
...
// const getHeadToHead_APi = async (playerId, opponentId, setH2Hstate) => {
//   setH2Hstate({
//     error: "",
//     loading: true,
//     data: {},
...
//   try {
//     const data = await axios.post("/api/Players/getHeadToHeadStats", {
//       playerId,
//       opponentId,
//     })
...
export function usePlayerStats(playerId) {
  ...
  const player = useMemo(() => {
    return players.find((item) => item?._id === playerId) || null
  }, [players, playerId])
  ...
  const [H2Hstate, setH2Hstate] = useState({
    error: "",
    loading: false,
    data: {},
  })
  ...
  useEffect(() => {
    if (player) {
      ...
      setProfileImage(player?.avatar || player?.image || "")
    }
  }, [player])
  ...
  }, [players, playerId, search])

  async function getHeadToHead_APi(opponentId) {
    ...
    try {
      const response = await axios.post(
        "/api/Players/getHeadToHeadStats",
        {
          playerId,
          opponentId,
  ...
  async function selectOpponent(opponent) {
    setSelectedOpponent(opponent)
    setSearch(opponent?.name || "")

    await getHeadToHead_APi(opponent?._id)
  }
  ...
  return {
    ...
    headToHead: H2Hstate.data,
    H2Hloading: H2Hstate.loading,
    H2Herror: H2Hstate.error,

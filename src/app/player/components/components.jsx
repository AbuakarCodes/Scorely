...
export function StatCard({ label, value, highlight = false, className = "" }) {
  return (
    <div
      ...
      `}
    >
      <p className="text-[10px] uppercase tracking-[0.16em] font-bold text-[#586377] mb-2">
        {label}
      </p>
  ...
  return (
    <section className="relative overflow-hidden bg-[#f2f4f6] pt-12 pb-24">
        ...
        <div className="flex flex-col items-start gap-10 lg:flex-row lg:items-end">
          ...
          <div className="pb-3">
            ...
            <div className="mt-7 flex flex-wrap items-center gap-7">
              <div>
                <p className="mb-1 text-[10px] uppercase tracking-[0.2em] text-[#586377]">
                  Primary Role
                </p>

                <p className="text-xl font-bold italic text-[#003527]">
                  {player.role}
                </p>
              </div>
      ...
      <div className="pointer-events-none absolute right-0 top-0 h-full w-1/2 opacity-[0.04]">
        <svg
          viewBox="0 0 100 100"
          className="h-full w-full fill-[#003527]"
        >
...
export function SectionHeading({ number, title, description }) {
  return (
    <div className="mb-8 flex items-end justify-between px-2">
      ...
      <span className="hidden text-5xl font-black text-[#003527]/10 md:block">
        {number}
      </span>
    </div>
  );
...
export function BattingStats({ stats }) {
  ...
  <SectionHeading
    ...
    description="Complete batting statistics"
  />
  ...
  <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-[#e0e3e5] shadow-sm md:grid-cols-4 lg:grid-cols-6">
    ...
    <div className="col-span-2 flex flex-col justify-between bg-[#064e3b] p-7 sm:p-8">
      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#b0f0d6]">
        Total Runs
      </span>
    ...
    <StatCard
      label="Boundary %"
      value={`${stats?.boundaryPercentage ?? 0}%`}
    />
  ...
  return (
    <section className="mb-16">
      <SectionHeading
        ...
        description="Complete bowling statistics"
      />
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-[#e0e3e5] shadow-sm md:grid-cols-4 lg:grid-cols-6">
        ...
        <div className="col-span-2 flex flex-col justify-between bg-[#545f73] p-7 sm:p-8">
          ...
          <div>
            ...
            <p className="mt-2 text-xs uppercase tracking-widest text-[#d8e3fb]/70">
              {stats.overs} Overs Bowled
            </p>
          </div>
        </div>

        <StatCard label="Economy" value={stats.economy} highlight />
        <StatCard label="Bowling Average" value={stats.average} />
        <StatCard label="Bowling SR" value={stats.strikeRate} />
        ...
        <StatCard label="Runs Conceded" value={stats.runsConceded} />
        <StatCard label="Maidens" value={stats.maidens} />
        <StatCard label="Dot Balls" value={stats.dotBalls} />
        <StatCard label="Wides" value={stats.wides} />
        <StatCard label="No Balls" value={stats.noBalls} />
        <StatCard
          label="Boundaries Conceded"
          value={stats.boundariesConceded}
        />
      </div>
    </section>
  );
...
export function H2HStat({ label, value, emphasis = false }) {
  return (
    <div className="rounded-xl bg-[#f2f4f6] p-4 sm:p-5">
      <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-[#586377]">
        {label}
      </p>
        ...
        <div className="mb-10 flex flex-col justify-between gap-7 lg:flex-row lg:items-center">
          ...
          <div className="relative w-full lg:w-96">
            <div className="relative">
              <Search
                size={19}
                className="absolute left-4 top-1/2 -translate-y-1/2 text-[#586377]"
              />
            ...
            <div className="grid gap-10 lg:grid-cols-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <H2HStat
                    label="Runs"
                    value={headToHead.batting.runs}
                    emphasis
                  />

                  <H2HStat
                    label="Balls Faced"
                    value={headToHead.batting.ballsFaced}
                  />

                  <H2HStat
                    label="Strike Rate"
                    value={headToHead.batting.strikeRate}
                  />

                  <H2HStat
                    label="Average"
                    value={headToHead.batting.average}
                  />

                  <H2HStat
                    label="4s"
                    value={headToHead.batting.fours}
                  />

                  <H2HStat
                    label="6s"
                    value={headToHead.batting.sixes}
                  />

                  <H2HStat
                    label="Dismissals"
                    value={headToHead.batting.dismissals}
                  />

                  <H2HStat
                    label="Dot Balls"
                    value={headToHead.batting.dotBalls}
                  />

                  <H2HStat
                    label="Boundary %"
                    value={`${headToHead.batting.boundaryPercentage}%`}
                  />

                  <H2HStat
                    label="Highest Score"
                    value={headToHead.batting.highestScore}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <H2HStat
                    label="Overs"
                    value={headToHead.bowling.overs}
                    emphasis
                  />

                  <H2HStat
                    label="Balls"
                    value={headToHead.bowling.balls}
                  />

                  <H2HStat
                    label="Runs Conceded"
                    value={headToHead.bowling.runsConceded}
                  />

                  <H2HStat
                    label="Wickets"
                    value={headToHead.bowling.wickets}
                    emphasis
                  />

                  <H2HStat
                    label="Economy"
                    value={headToHead.bowling.economy}
                  />

                  <H2HStat
                    label="Bowling SR"
                    value={headToHead.bowling.strikeRate}
                  />

                  <H2HStat
                    label="Average"
                    value={headToHead.bowling.average}
                  />

                  <H2HStat
                    label="Dot Balls"
                    value={headToHead.bowling.dotBalls}
                  />

                  <H2HStat
                    label="4s Conceded"
                    value={headToHead.bowling.foursConceded}
                  />

                  <H2HStat
                    label="6s Conceded"
                    value={headToHead.bowling.sixesConceded}
                  />

                  <H2HStat
                    label="Wides"
                    value={headToHead.bowling.wides}
                  />

                  <H2HStat
                    label="No Balls"
                    value={headToHead.bowling.noBalls}
                  />

                  <H2HStat
                    label="Best Figures"
                    value={headToHead.bowling.bestFigures}
                  />
                </div>
              </div>
            </div>

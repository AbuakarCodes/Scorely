  ...
  reducers: {
    ...
    chnageBatsmen_OR_Bowler(state, action) {
      ...
      const needBothBatsmen =
        state.innings.pendingNewBatsman.striker && state.innings.pendingNewBatsman.nonStriker

      const needStriker = state.innings.pendingNewBatsman.striker && !needBothBatsmen

      const needNonStriker = state.innings.pendingNewBatsman.nonStriker && !needBothBatsmen
    ...
    update_pendingPlayersFlag(state, action) {
      ...
      const arePlayersLeft = numberOfPlayersCanComeToCrease({
        team,
        tossDecision,
        tossWinner,
        isFirstInings,
      ...
      if (isWicket && arePlayersLeft != 0)
        state.innings.pendingNewBatsman = { ...state.innings.pendingNewBatsman, striker: true }
    ...
    handelLastPlayer_isLastPlayerTrue(state, action) {
      ...
      const isFirstInings = state?.innings?.isFirstInings
      const isWicket = ballObject?.isWicket
      const canContinueBat = numberOfPlayersCanComeToCrease({
        team,
        tossDecision,
        tossWinner,
        isFirstInings,
    ...
    update_CRRandRRR(state, action) {
      ...
      const RRR =
        isFirstInings === false ? calculateRRR({ target, currentRuns, over, ballsInOver, TotalOvers }) : 0
    ...
    Update_Strike(state, action) {
      ...
      if ((isOddRuns && !isLastBall) || (!isOddRuns && isLastBall)) {
    ...
    update_isDissmissedFlag(state, action) {
      ...
      const striker = [...state?.match?.teams?.teamA?.players, ...state?.match?.teams?.teamB?.players]?.find(
        (p) => p?._id === ballObject?.strikerId,
      )
      if (!striker) return
    ...
    switchSides(state, action) {
      ...
      const batingTeamPlayersWraper = () =>
        batting_bowlingTeam({ team, tossDecision, tossWinner, isFirstInings: false })?.battingTeam?.players ??
        []
      // parameters
      const team = state?.match?.teams || []
      const tossDecision = state?.match?.tossDecision || ""
      ...
      const battingTeamPlayers =
        batting_bowlingTeam({ team, tossDecision, tossWinner, isFirstInings })?.battingTeam?.players || []
    ...
    match_Decision(state, action) {
      ...
      const battingTeamPlayers =
        batting_bowlingTeam({ team, tossDecision, tossWinner, isFirstInings: state?.innings?.isFirstInings })
          ?.battingTeam?.players || []
      const secondInningsWickets = getDismissedBattersCount(battingTeamPlayers)
      ...
      if (runs >= target) {
        const battingTeam = batting_bowlingTeam({
          ...
          tossDecision,
          tossWinner,
          isFirstInings: state?.innings?.isFirstInings,
        })?.battingTeam ?? { id: "", name: "" }
        ...
        if (runs === target - 1) {
          ...
          const bowlingTeam = batting_bowlingTeam({
            team,
            tossDecision,
            tossWinner,
            isFirstInings: state?.innings?.isFirstInings,
...
function swapStriker(batsmen) {
  const strikerKey = Object.keys(batsmen).find((key) => batsmen[key].isStriker)

  const nonStrikerKey = Object.keys(batsmen).find((key) => !batsmen[key].isStriker)
...
function hasOversCompleted({ over, TotalOvers, isLegalDelivery, ballInOver }) {
  if (over == null || TotalOvers == null || ballInOver == null || typeof isLegalDelivery !== "boolean")
    return false
...
function numberOfPlayersCanComeToCrease({ team, tossDecision, tossWinner, isFirstInings, ballObject }) {
  ...
  const remaningPlayers = battingTeam?.players?.filter(
    (p) => !p?.isDismissed && p?._id !== ballObject?.strikerId && p?._id !== ballObject?.nonStrikerId,
  ).length
...
function calculateRRR({ target, currentRuns, over, ballsInOver, TotalOvers }) {
  if (target == null || currentRuns == null || over == null || ballsInOver == null || TotalOvers == null)
    return 0
